...
class SessionRecovery:
            ...
            plan.append(
                {
                    "step": len(plan) + 1,
                    "action": "🧪 修复失败的测试",
                    "command": "pytest tests/ -v --tb=short --lf",
                    "priority": "high",
    ...
    def _check_git_state(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": status_output[:100] + "..."
                if len(status_output) > 100
                else status_output,
    ...
    def _check_ruff_state(self) -> dict[str, Any]:
        ...
        try:
            ...
            version_result = subprocess.run(
                ["ruff", "--version"], capture_output=True, text=True, cwd=self.project_root
            )
    ...
    def _check_coverage_state(self) -> dict[str, Any]:
        ...
        try:
            # 检查 coverage 是否安装
            version_result = subprocess.run(
                ["coverage", "--version"], capture_output=True, text=True, cwd=self.project_root
            )
            if version_result.returncode != 0:
        ...
        try:
            ...
            if checkpoint_dirs:
                checkpoint_dirs.sort(
                    key=lambda x: (checkpoints_dir / x).stat().st_mtime, reverse=True
                )
    ...
    def print_recovery_guide(self, verbose: bool = False) -> None:
        ...
        print("\n📊 项目状态概览:")
        print(
            f"  Git分支: {state['git']['branch']} ({'有未提交更改' if state['git']['has_changes'] else '干净'})"
        )
        ...
        print(
            f"  GUI版本: {state['gui_version']['status']} (核心: {state['gui_version']['core']}, GUI: {state['gui_version']['gui']})"
        )

        if state.get("mission"):
            print(
                f"  任务进度: {state['mission'].get('progress', '0%')} - {state['mission'].get('current_milestone', '未知')}"
            )
        ...
        if verbose:
            ...
            if state["tests"]["available"]:
                print(
                    f"  测试详情: {state['tests']['passed']}通过/{state['tests']['failed']}失败/{state['tests']['skipped']}跳过/{state['tests']['errors']}错误"
                )
        ...
        print(
            f"  2. 查看详细计划: cat {self.mission_dir}/features.json | jq '.milestones[] | .name, .features[].name'"
        )
        print("  3. 运行验证检查: python scripts/session_recovery.py --check")
        print("  4. 查看下一步建议: python scripts/session_recovery.py --next")

        # 基于状态的特定建议
        if state["mission"].get("current_milestone") == "m1":
            print("\n💡 基于 Phase A (技术债清零) 的建议:")
            print("  - 优先执行 Ruff 修复: ruff check src/ --fix --unsafe-fixes")
            print("  - 验证测试: pytest tests/ -v --tb=short --maxfail=1")
            print("  - 对齐版本: 更新 gui/src-tauri/tauri.conf.json")
    ...
    def run_verification_checks(self) -> None:
        ...
        if state["gui_version"]["aligned"]:
            ...
            checks.append(
                (
                    "❌ 版本对齐",
                    f"核心: {state['gui_version']['core']}, GUI: {state['gui_version']['gui']}",
                )
            )
    ...
    def suggest_next_steps(self) -> None:
        ...
        if not current_milestone or current_milestone == "m0":
            ...
            print("  查看详细任务计划:")
            print(
                f"     cat {self.mission_dir}/features.json | jq -r '.milestones[] | select(.id==\"{current_milestone}\") | .name, .features[].name'"
            )

        print("\n💡 通用建议:")
        print("  - 每次会话结束时创建检查点")
        print("  - 使用 git commit 保存工作进度")
        print("  - 运行 python scripts/session_recovery.py 验证状态")
