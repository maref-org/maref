...
def run_phase23(rounds, harness, tracker):
    ...
    axes_configs = [
        (
            "R77",
            {"agent_concurrency": 400, "churn_rate": 100},
            [400, 500, 600, 700, 800],
            "agent_concurrency",
        ),
    ...
    for rid, base_axes, values, vary_axis in axes_configs:
        for v in values:
            ...
            print(
                f"  {label:35s} score={result.resilience_score:6.2f}  p99={result.latency_p99:7.2f}ms  "
...
def run_phase45(harness, tracker):
    ...
    rounds = [
        ...
        (
            "R91-政策地震",
            {"churn_rate": 200, "fault_rate": 10, "oscillation_rate": 30, "data_volume": 500000},
            1.0,
        ),
        (
            "R92-全面战争",
    ...
    for rid, axes, duration in rounds:
        ...
        print(
            f"  {rid:35s} score={result.resilience_score:6.2f}  p99={result.latency_p99:7.2f}ms  "
...
def run_r95_soak(harness, tracker, hours: float = 0.5):
    ...
    print(
        f"  R95-soak score={result.resilience_score:.2f}  p99={result.latency_p99:.2f}ms  "
...
def run_r98_fuzz(tracker):
    ...
    for i in range(10000):
        try:
            ...
            r.evaluate(
                {"survival_rate": random.random(), "recovery_time_ms": random.uniform(1, 10000)}
            )
            total += 1
        except Exception:
            errors += 1
