...
def monitor_one_account(account: dict[str, str], rows: list[dict[str, str]]) -> None:
    ...
    with imaplib.IMAP4_SSL(server, 993, ssl_context=context) as imap:
        ...
        for msg_id in ids:
            ...
            if should_report_message(sender, subject, targets):
                print(f"[{time.strftime('%H:%M:%S')}] {username} NEW: {decode_mime(msg.get('From'))} | {subject}")
...
def monitor_inbox(rows: list[dict[str, str]], interval: int, loops: int) -> int:
    ...
    for loop in range(loops):
        for account in accounts:
            try:
                monitor_one_account(account, rows)
            except Exception as exc:
                print(f"[{time.strftime('%H:%M:%S')}] {account['username']} monitor error: {type(exc).__name__}: {exc}", file=sys.stderr)
        if loop < loops - 1:
            time.sleep(interval)
    return 0
