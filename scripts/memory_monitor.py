    ...
    for proc in processes:
        if proc["rss_mb"] > threshold_mb:
            ...
            if not is_critical and is_maref:
                try:
                    log(f"终止内存过高的进程: PID={proc['pid']}, RSS={proc['rss_mb']:.1f}MB, CMD={proc['command'][:80]}")
...
def monitor_loop() -> None:
    ...
    while True:
        try:
            ...
            log(f"内存使用: {used:.1f}% | 空闲: {free_mb:.0f}MB | 压缩: {mem.get('compressor_mb', 0):.0f}MB")
            
            # 检查 maref serve 运行时间
            long_running = check_maref_serve_uptime()
            for proc in long_running:
                log(f"⚠️  maref serve 已运行 {proc['uptime_hours']:.1f} 小时，建议重启 (PID: {proc['pid']})")
            ...
            # 内存警告 - 显示 Top 5 进程
                ...
                for i, proc in enumerate(procs[:5], 1):
                    log(f"  {i}. PID={proc['pid']} RSS={proc['rss_mb']:.0f}MB MEM={proc['mem']:.1f}% CMD={proc['command'][:60]}")
...
if __name__ == "__main__":
    if "--once" in sys.argv:
        ...
        for i, proc in enumerate(get_process_memory()[:5], 1):
            print(f"  {i}. PID={proc['pid']} RSS={proc['rss_mb']:.0f}MB MEM={proc['mem']:.1f}% CMD={proc['command'][:80]}")
        
        print("\nmaref serve 运行时间检查:")
        long_running = check_maref_serve_uptime()
        if long_running:
            for proc in long_running:
                print(f"  ⚠️  PID={proc['pid']} 运行 {proc['uptime_hours']:.1f} 小时 RSS={proc['rss_mb']:.0f}MB")
        else:
