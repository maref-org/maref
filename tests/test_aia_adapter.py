...
class TestVerifyCAI:
    def test_verify_valid_cai(self) -> None:
        """测试合法 CAI 证书验证."""
        cai_plaintext = (
            f"AIC-12345:{SM2_PUBLIC_KEY}:CASP-001:1700000000:1800000000"
        ).encode()
        signature = sm2_sign(
...
class TestCertificateVerify:
    def test_generate_and_verify(self) -> None:
        ...
        signature = generate_certificate_verify(
            SM2_PRIVATE_KEY, SM2_PUBLIC_KEY, handshake_messages
        )

        assert verify_certificate_verify(
            SM2_PUBLIC_KEY, handshake_messages, signature
        )
    ...
    def test_verify_tampered_messages(self) -> None:
        ...
        assert not verify_certificate_verify(
            SM2_PUBLIC_KEY, b"tampered", signature
        )
