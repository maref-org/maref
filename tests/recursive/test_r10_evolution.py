...
class TestSafetyGate:
    def test_safe_proposal_passes(self) -> None:
        gate = SafetyGate()
        rule = EvolutionRule(
            rule_id="r1", target="adoption_gain_threshold",
            current_value=0.05, proposed_value=0.03,
            justification="降低阈值",
        )
    ...
    def test_delete_cb_is_rejected(self) -> None:
        gate = SafetyGate()
        rule = EvolutionRule(
            rule_id="r2", target="circuit_breaker",
            current_value="active", proposed_value=None,
            justification="移除 CB",
        )
        result = gate.evaluate(rule)
    ...
    def test_delete_state_machine_rejected(self) -> None:
        gate = SafetyGate()
        rule = EvolutionRule(
            rule_id="r3", target="state_machine",
            current_value="active", proposed_value=None,
        )
        result = gate.evaluate(rule)
        assert result.passed is False
    ...
    def test_non_core_set_to_none_passes(self) -> None:
        gate = SafetyGate()
        rule = EvolutionRule(
            rule_id="r4", target="debug_mode",
            current_value=True, proposed_value=None,
        )
        result = gate.evaluate(rule)
        assert result.passed is True
...
class TestEvolutionDSL:
    ...
    def test_propose_creates_rule(self, dsl: EvolutionDSL) -> None:
        rule = dsl.propose("heal_max_iterations", 3, 5,
                            "允许更多修复迭代")
    ...
    def test_safety_check_dangerous_proposal(self, dsl: EvolutionDSL) -> None:
        with dsl.freeze_zone.temporary_override("circuit_breaker"):
            rule = dsl.propose("circuit_breaker", "active", None,
                                "CB 阻碍性能")
            result = dsl.safety_check(rule)
            assert result.passed is False
    ...
    def test_safety_gate_with_test_metrics_pass(self) -> None:
        ...
        rule = EvolutionRule(
            rule_id="r_test", target="test_param",
