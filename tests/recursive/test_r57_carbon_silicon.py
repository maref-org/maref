...
class TestTaskAllocation:
    def test_allocate_normal_task(self):
        css = CarbonSiliconSymbiosis()
        task = css.allocate_task(
            TaskDomain.CODE_GENERATION, "agent_1", "Generate Code", "Generate a utility module"
        )
    ...
    def test_allocate_security_task(self):
        css = CarbonSiliconSymbiosis()
        task = css.allocate_task(
            TaskDomain.SECURITY_REVIEW, "agent_1", "Security Audit", "Review security policies"
        )
    ...
    def test_old_yang_architecture_collaborative_to_agent_only(self):
        ...
        css.set_agent_trust("agent_1", 0.95)
        task = css.allocate_task(
            TaskDomain.ARCHITECTURE_DESIGN, "agent_1", "Design System", "Design new module"
        )
        assert task.allocation == TaskAllocation.AGENT_ONLY
...
class TestWorkflowLifecycle:
    def test_start_workflow(self):
        css = CarbonSiliconSymbiosis()
        instance = css.start_workflow(
            "agent_1", TaskDomain.CODE_GENERATION, "Generate Code", "Description"
        )
    ...
    def test_human_confirm(self):
        css = CarbonSiliconSymbiosis()
        instance = css.start_workflow(
            "agent_1", TaskDomain.ARCHITECTURE_DESIGN, "Design", "Description"
        )
    ...
    def test_agent_execute(self):
        css = CarbonSiliconSymbiosis()
        instance = css.start_workflow(
            "agent_1", TaskDomain.CODE_GENERATION, "Generate", "Description"
        )
    ...
    def test_agent_self_review(self):
        css = CarbonSiliconSymbiosis()
        instance = css.start_workflow("agent_1", TaskDomain.MONITORING, "Monitor", "Description")
...
class TestWorkflowQuery:
    def test_get_workflow_by_id(self):
        css = CarbonSiliconSymbiosis()
        instance = css.start_workflow("agent_1", TaskDomain.CODE_GENERATION, "Test", "Desc")
...
class TestSerialization:
    def test_workflow_task_to_dict(self):
        task = WorkflowTask(
            "t1", "Title", "Desc", TaskDomain.CODE_GENERATION, TaskAllocation.AGENT_ONLY
        )
        d = task.to_dict()
