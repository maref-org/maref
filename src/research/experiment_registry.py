...
class ExperimentMetadata:
    ...
    novelty_score: float = 0.5  # 0-1, higher = more likely to produce new findings
...
class ExperimentRegistry:
    ...
    def _register_default_experiments(self) -> None:
        ...
        self.register(
            ...
            ExperimentMetadata(
                name="random_walk",
                phase=8,
                description="随机状态转移路径分析",
                expected_duration_ms=50.0,
            ),
        )
        self.register(
            "gray_code_fault_tolerance",
            self._run_gray_code_test,
            ExperimentMetadata(
                name="gray_code_fault_tolerance",
                phase=8,
                description="格雷码单比特容错测试",
        ...
        self.register(
            "policy_lifecycle",
            self._run_policy_lifecycle,
            ExperimentMetadata(
                name="policy_lifecycle",
                phase=9,
                description="策略变更生命周期测试",
                expected_duration_ms=150.0,
            ),
        )
        self.register(
            "rollback_safety",
            self._run_rollback_safety,
            ExperimentMetadata(
        ...
        self.register(
            ...
            ExperimentMetadata(
                name="ab_test_winner",
                phase=9,
                description="A/B测试胜出者选择",
                expected_duration_ms=120.0,
            ),
        )
        self.register(
            "degradation_prevention",
            self._run_degradation_prevention,
            ExperimentMetadata(
                name="degradation_prevention",
                phase=9,
                description="退化预防测试",
                expected_duration_ms=100.0,
            ),
        ...
        self.register(
            ...
            self._run_meta_convergence,
            ExperimentMetadata(
                name="meta_learning_convergence",
                phase=10,
                description="元学习器收敛测试",
                expected_duration_ms=300.0,
            ),
        )
        self.register(
            "weight_stability",
            self._run_weight_stability,
            ExperimentMetadata(
                name="weight_stability",
                phase=10,
                description="极端条件下权重稳定性",
                expected_duration_ms=250.0,
            ),
        )
        self.register(
            "recursive_safety",
            self._run_recursive_safety,
            ExperimentMetadata(
    ...
    def update_metadata(self, name: str, findings: int, duration_ms: float) -> None:
        ...
        # Update novelty (decay over time)
    ...
    # Experiment runners (wrappers around existing code)
    # P0.1: All experiments now use real LLM calls via the research classes
