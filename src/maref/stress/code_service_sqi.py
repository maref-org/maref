...
class CodeServiceSQI(ServiceQualityIndex):
    ...
    def compute(
        ...
        """
        has_harness_data = stress_result is not None or emergence_report is not None or budget_usage_pct > 0
        ...
        if has_harness_data:
            ...
            # Code-only mode: only code-specific dimensions (6 dims)
            dimensions = [
                self._compute_test_coverage(code_metrics),
                self._compute_lint_pass(code_metrics),
                self._compute_build_success(code_metrics),
            ...
            # Re-normalize weights to sum to 1.0 for code-only dimensions
            code_dim_names = {d.name for d in dimensions}
            total_code_weight = sum(
                w for name, w in self._weights.items() if name in code_dim_names
            )
            if total_code_weight > 0:
                for d in dimensions:
                    d.weight = self._weights.get(d.name, 0.1) / total_code_weight

        # Apply current weight configuration (for full mode)
        if has_harness_data:
            for d in dimensions:
        ...
        mode = "code_only" if not has_harness_data else "full"
        return SQIReport(
            dimensions=dimensions,
            overall_score=overall,
            variance=variance,
            round_id=round_id,
            metadata={"weight_profile": "custom" if self._weights != WEIGHT_PROFILES.get("default") else "default", "mode": mode},
        )
