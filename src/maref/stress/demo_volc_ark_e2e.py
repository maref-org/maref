"""End-to-end verification: VolcArkCodeAgent + dynamic SQI + aggressive convergence.

Uses Volcengine Ark Coding Plan (Anthropic-compatible API):
  base_url: https://ark.cn-beijing.volces.com/api/plan
  api_key: ark-xxx
  models: ark-code-latest (Auto 路由)

Addresses:
  Q1: Real code generation via Volcengine Ark API
...
def demo_q1_real_code_generation() -> dict:
    """Q1: Verify real code generation via Volcengine Ark API."""
    print("\n" + "=" * 70)
    print("Q1: Real Code Generation via Volcengine Ark Coding Plan")
    print("=" * 70)
    ...
    try:
        ...
        for task in CODE_PROMPTS:
            ...
            result = agent.generate_with_retry(
                prompt=task["prompt"],
                language="python",
                temperature=0.0,  # Deterministic for faster, consistent output
                max_retries=1,
            )
...
def demo_q2_dynamic_sqi_weights() -> dict:
    ...
    for profile_name in WEIGHT_PROFILES:
        ...
        top_dim = max(report.dimensions, key=lambda d: d.score * d.weight)
        lowest_dim = min(report.dimensions, key=lambda d: d.score * d.weight)

        profile_results[profile_name] = {
            "overall_score": round(report.overall_score, 2),
            "variance": round(report.variance, 2),
            "top_dimension": top_dim.name,
            "lowest_dimension": lowest_dim.name,
        }

        print(f"\n  {profile_name}:")
        print(f"    SQI: {report.overall_score:.1f}  (variance: {report.variance:.1f})")
        print(f"    Top: {top_dim.name}  |  Lowest: {lowest_dim.name}")
...
def demo_q3_aggressive_convergence() -> dict:
    ...
    print("\n  Round | SQI   | Delta  | Success | Coverage | Strategy")
    print("  " + "-" * 62)
...
def demo_full_integration() -> dict:
    ...
    print("\n" + "=" * 70)
    print("Volcengine Ark + MAREF: End-to-End Verification")
    print("=" * 70)
...
if __name__ == "__main__":
    print("MAREF + Volcengine Ark Code Service Factory")
    print("Base URL: https://ark.cn-beijing.volces.com/api/plan")
    print("Model: ark-code-latest")
