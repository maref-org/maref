...
class ASTNormalizer:
    ...
    def _normalize_node(self, node: ast.AST) -> ast.AST:
        ...
        if isinstance(node, ast.Name):
            ...
            return ast.For(
                target=cast(ast.expr, self._normalize_node(node.target)),
            ...
            return ast.Expr(value=cast(ast.expr, self._normalize_node(node.value)))
    ...
    def _normalize_arguments(self, node: ast.arguments) -> ast.arguments:
        ...
        return ast.arguments(
            posonlyargs=[cast(ast.arg, self._normalize_node(arg)) for arg in node.posonlyargs],
    ...
    def _generic_normalize(self, node: ast.AST) -> ast.AST:
        ...
        for field_name, old_value in ast.iter_fields(node):
            if isinstance(old_value, ast.AST):
                new_value: Any = self._normalize_node(old_value)
            ...
            setattr(new_node, field_name, new_value)
    ...
    def _extract_tokens(self, node: ast.AST) -> list[str]:
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name):
                ...
                tokens.append(f"CONST:{child.value!r}")
