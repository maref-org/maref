...
class RedactionEngine:
    ...
    def _apply_redaction(self, image: Image.Image, zone: RedactionZone) -> Image.Image:
        ...
        if zone.mode == RedactionMode.BLACK_BOX:
            ...
            small = region.resize((8, 8), resample=Image.NEAREST)  # type: ignore
            pixelated = small.resize(region.size, Image.NEAREST)  # type: ignore
    ...
    def _capture(
        ...
        if image is not None and self.downsample_factor < 1.0:
            ...
            if self.downsample_method == DownsampleMethod.BILINEAR:
                image = image.resize((new_w, new_h), Image.BILINEAR)  # type: ignore
            elif self.downsample_method == DownsampleMethod.LANCZOS:
                image = image.resize((new_w, new_h), Image.LANCZOS)  # type: ignore
            else:
                image = image.resize((new_w, new_h), Image.NEAREST)  # type: ignore
