...
A session is a bounded execution context where SEMI_TRUSTED agents can run
autonomous task chains without per-action HITL blocking. Within a session:
...
_VALID_SESSION_AGENTS = {"SEMI_TRUSTED"}
...
def declare_session(
    ...
    Raises ValueError if the agent's trust_level is not SEMI_TRUSTED.
    """
