...
_EVENT_STATE_MAP: dict[PERCVEventType, tuple[GovernanceState | None, str]] = {
    PERCVEventType.RESEARCH_START: (GovernanceState.ANALYZE, "transition"),
    PERCVEventType.RESEARCH_COMPLETE: (GovernanceState.REPORT, "transition"),
    PERCVEventType.RESEARCH_FAIL: (GovernanceState.HALT, "force_halt"),
    PERCVEventType.BUDGET_WARNING: (GovernanceState.STABILIZE, "force_stabilize"),
    PERCVEventType.BUDGET_CRITICAL: (GovernanceState.HALT, "force_halt"),
...
class PERCVGovernanceHook:
    ...
    def _default_handle(
        ...
        if action == "force_halt" and target_state:
        ...
        if action == "force_stabilize" and target_state:
            reason = f"{event_type.value}:{next(iter(payload.values()), '')}"
            self.state_machine.force_stabilize(reason)
