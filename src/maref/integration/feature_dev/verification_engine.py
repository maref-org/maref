...
from maref.integration.feature_dev.progress_tracker import ConvergenceReport, ProgressTracker
...
class DeliveryVerifier:
    """Auto-verify against the delivery standards in the AI-Native IP Kingdom doc.

    Checks:
    - §1.5: All 3 hypotheses extracted
    - §1.6: Go/No-Go mechanism present
    - §6: Daily/weekly/monthly checklists extracted
    - §7: Cost models extracted
    - §9: 6 disciplines extracted
    - §2/§3: All 3 stages detected
    - Pipeline: Deploy readiness after 10 rounds
    """
    ...
    def _all_flat(self, sections: list) -> list:
        ...
        for sec in sections:
            ...
            for sub in getattr(sec, "subsections", []):
                result.extend(self._all_flat([sub]))
        return result

    def verify(self, report: ConvergenceReport | None = None) -> DeliveryVerdict:
        self.items = []
        self._check_hypotheses()
        self._check_go_nogo()
        self._check_stages()
        self._check_checklists()
        self._check_cost_models()
        self._check_disciplines()
        self._check_deploy_readiness(report)
        self._check_convergence(report)
        self._check_methods_tracked(report)
        ...
        summary = (
            ...
            f"score={score:.1f}% ({passed_count}/{total_count} checks passed). "
            f"Deploy readiness: {'YES' if report and report.deploy_ready else 'NO' if report else 'N/A'}. "
            f"Decision: {report.final_decision if report else 'N/A'}."
        )
...
def run_verification_pipeline(doc_path: str, iterations: int = 10) -> dict[str, Any]:
    ...
    tracker = ProgressTracker(feature_name=doc.title)
