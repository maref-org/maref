...
def _benchmark(
    name: str, op: str, fn: Callable[[], object], iterations: int, data_bytes: int = 0
) -> BenchmarkResult:
    """运行基准测试."""
    ...
    throughput = (
        data_bytes * iterations / elapsed / 1048576 if elapsed > 0 and data_bytes > 0 else None
    )
...
def run_all_benchmarks(
    sm2_keypair: SM2KeyPair | None = None, data_size_bytes: int = 1024, iterations: int = 100
...
def format_results(results: list[BenchmarkResult]) -> str:
    ...
    lines = [
        "=" * 80,
        f"{'Algorithm':<15} {'Operation':<20} {'Ops/sec':>12} {'Throughput':>15} {'Total(s)':>10}",
        "-" * 80,
    ]
    for r in results:
        tp = f"{r.throughput_mbps:.2f} MB/s" if r.throughput_mbps is not None else "N/A"
        lines.append(
            f"{r.algorithm:<15} {r.operation:<20} {r.ops_per_second:>12.1f} {tp:>15} {r.total_seconds:>10.3f}"
        )
    lines.append("=" * 80)
    return "\n".join(lines)
