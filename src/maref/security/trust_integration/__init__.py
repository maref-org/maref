...
class ChainRiskAnalyzer:
    ...
    def __init__(self, max_chain_depth: int = 5):
        ...
        self.risk_patterns = {
            "depth_exceeded": {"pattern": "委托链深度超过最大限制", "risk_score": 0.9},
            "circular_delegation": {"pattern": "循环委托检测", "risk_score": 0.95},
            "cross_domain_without_auth": {"pattern": "跨信任域调用缺少认证", "risk_score": 0.8},
            "overly_nested": {
                "pattern": "过度嵌套委托",
    ...
    def analyze(
        self, chain: DelegationChain, boundary_manager: TrustBoundaryManager | None = None
    ) -> list[ChainRiskFactor]:
        """分析委托链风险"""
        risks = []
    ...
    def _analyze_cross_domain(
        self, chain: DelegationChain, boundary_manager: TrustBoundaryManager
    ) -> list[ChainRiskFactor]:
        """分析跨信任域调用风险"""
        risks = []
    ...
    def _analyze_nested_pattern(self, chain: DelegationChain) -> ChainRiskFactor | None:
        """分析嵌套模式风险"""
...
class IntegratedTrustEngine:
    ...
    def __init__(
        self, trust_engine: TrustEngineV2, chain_analyzer: ChainRiskAnalyzer | None = None
    ):
    ...
    def evaluate_with_chain(
        self, agent_id: str, chain: DelegationChain | None = None
    ...
    def generate_combined_report(
        ...
        report = {
            ...
            "chain_impact": {
                ...
                "risk_count": len(risks),
                "trust_penalty": self.chain_analyzer.calculate_trust_penalty(risks)
                if risks
                else 0.0,
            },
        }
...
class TrustIntegrationAPI:
    ...
    def get_trust_score(self, agent_id: str, chain_data: dict | None = None) -> dict[str, Any]:
        """获取信任评分API"""
