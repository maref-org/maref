...
class ConsistencyCost:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'level': self.level.value, 'latency_ms': self.latency_ms, 'communication_multiplier': self.communication_multiplier, 'description': self.description}
...
class CostEstimator:
    ...
    def compare(cls, a: ConsistencyLevel, b: ConsistencyLevel) -> dict[str, Any]:
        ca = cls.estimate(a)
        cb = cls.estimate(b)
        return {'from': a.value, 'to': b.value, 'latency_delta_ms': round(cb.latency_ms - ca.latency_ms, 1), 'comm_ratio': round(cb.communication_multiplier / ca.communication_multiplier, 2)}
...
class DynamicDegrader:
    ...
    def __init__(self, high_load_threshold: float=0.8, critical_path_levels: dict[str, ConsistencyLevel] | None=None) -> None:
        self._high_load_threshold = high_load_threshold
        self._critical_path_levels = critical_path_levels or {}

    def resolve(self, step_id: str, requested: ConsistencyLevel, current_load: float, is_critical: bool=False) -> ConsistencyLevel:
    ...
    def explain(self, step_id: str, requested: ConsistencyLevel, current_load: float, is_critical: bool=False) -> dict[str, Any]:
        ...
        return {'step_id': step_id, 'requested': requested.value, 'effective': effective.value, 'is_critical': is_critical, 'current_load': round(current_load, 2), 'latency_before_ms': cost_before.latency_ms, 'latency_after_ms': cost_after.latency_ms, 'reason': 'load below threshold' if effective == requested else 'degraded due to high load'}