...
def generate_comparison_report(
    ...
    lines.append("")
    header = (
        f"{'Agent':<20} {'Rounds':>6} {'Converge@':>9} {'Oscillate':>9} "
        f"{'Time(s)':>8} {'FNR min':>7} {'FNR max':>7} {'Defects':>8} {'Fixes':>8}"
    )
    lines.append(header)
    lines.append("-" * len(header))

    for r in results:
        fnrs = r.fnr_trajectory()
        fnr_min = min(fnrs) if fnrs else 0
        fnr_max = max(fnrs) if fnrs else 0
        defects_ok = "✅" if r.acceptance.get("defects_detected") else "❌"
        fixes_ok = "✅" if r.acceptance.get("fixes_confirmed") else "❌"
        conv = str(r.convergence_round) if r.convergence_round >= 0 else "N/A"

        lines.append(
            f"{r.agent_name:<20} {r.rounds_completed:>6} {conv:>9} "
            f"{r.oscillation_count:>9} {r.total_time_s:>8.1f} "
            f"{fnr_min:>7.4f} {fnr_max:>7.4f} {defects_ok:>8} {fixes_ok:>8}"
        )
    lines.append("")
    ...
    for r in results:
        ...
        fnrs = r.fnr_trajectory()
        if fnrs:
            chart = _sparkline(fnrs)
            lines.append(f"FNR trajectory: {chart}")
            lines.append("")

        # Coverage trajectory
        covs = r.coverage_trajectory()
        if covs:
            chart = _sparkline(covs)
            lines.append(f"Coverage trajectory: {chart}")
            lines.append("")
...
def generate_json_report(results: list[SAEBResult], output_path: str | Path | None = None) -> str:
    data = {
        "comparison_date": __import__("time").time(),
...
def _sparkline(values: list[float], width: int = 20) -> str:
    ...
    return f"{result} [min={mn:.3f}, max={mx:.3f}]"
