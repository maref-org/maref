...
USE_CODEGEN_LOOP = (
    os.environ.get("MAREF_USE_CODEGEN_LOOP", "auto") in {"1", "true", "yes", "auto"}
    and _detect_llm_available()
)
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as _pool:
                    _future = _pool.submit(
                        lambda: asyncio.run(self._llm_generator.generate(proposal))
                    )
...
class SelfExecutor:
    def __init__(
        ...
        quality_gate: Callable[[GeneratedCode, ExecutionPipelineRecord], ExecutionResult]
        | None = None,
    ...
    def _default_quality_gate(
        ...
        if suffix == ".py":
            ...
            if rel_path is not None:
                ...
                pre_errors = (
                    _count_eslint_errors(backup_path)
    ...
    def _do_intent_drift_check(self, code: GeneratedCode) -> ExecutionResult | None:
        if self._intent_drift_detector is None:
            return None
        result = self._intent_drift_detector.evaluate_code(
            code.content, criteria=[], expected_hash=""
        )
        if result.blocked:
            return ExecutionResult(
                stage=ExecutionStage.SAFETY_GATE,
