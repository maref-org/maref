...
def _hmac_key() -> bytes:
    ...
    if _EPHEMERAL_KEY is None:
        ...
        logger.warning(
            "%s not set — using ephemeral key. HMACs will be invalid across restarts. Set %s for production.",
            _HMAC_KEY_ENV,
            _HMAC_KEY_ENV,
        )
...
class SecurityTemplateLib:
    ...
    def _init_builtins(self) -> None:
        bcrypt_tmpl = SecurityTemplate(
            domain="password_storage",
            description="Password storage: use bcrypt.hashpw + bcrypt.checkpw",
            template_code="import bcrypt\ndef hash_password(password: str) -> str:\n    salt = bcrypt.gensalt()\n    return bcrypt.hashpw(password.encode(), salt).decode()\ndef verify_password(password: str, hashed: str) -> bool:\n    return bcrypt.checkpw(password.encode(), hashed.encode())\n",
            blocked_keywords=["hashlib.md5", "hashlib.sha1", "crypt.md5", "md5(password"],
        )
        sql_tmpl = SecurityTemplate(
            domain="sql_query",
            description="SQL queries: use parameterized statements",
            template_code='# Safe: parameterized query\ncursor.execute(\n    "SELECT * FROM users WHERE id = ?",\n    (user_id,)\n)\n',
            blocked_keywords=['f"', "f'", ".format(", " % "],
        )
        https_tmpl = SecurityTemplate(
            ...
            for kw in node.keywords:
                if kw.arg == "verify":
                    ...
                    if (
                        ...
                        or (isinstance(val, ast.Name) and val.id == "False")
                    ):
                        verify_false = True
            if verify_false:
                violations.append(
