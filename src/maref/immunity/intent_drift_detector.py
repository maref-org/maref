...
class IntentDriftDetector:
    ...
    def evaluate_code(self, code: str, criteria: list[AcceptanceCriterion], expected_hash: str, immune_checker: ImmuneChecker | None=None, language: str='python') -> IntentDriftResult:
        ...
        return IntentDriftResult(passed=passed, intent_valid=True, test_results=test_results, blocked=False, extracted_gene_ids=extracted, immune_hits=immune_hits)

    def _fuzz_test_code(self, code: str, criteria: list[AcceptanceCriterion], language: str='python') -> list[FuzzTestResult]:
        if language != 'python':
            return self._fuzz_non_python(code, criteria, language)
        try:
            tree = ast.parse(code)
        except SyntaxError as e:
            return [FuzzTestResult(criterion_id=c.criterion_id, description=c.description, category=c.category, passed=False, error=f'Syntax error: {e}') for c in criteria]
    ...
    def _fuzz_non_python(self, code: str, criteria: list[AcceptanceCriterion], language: str) -> list[FuzzTestResult]:
        results: list[FuzzTestResult] = []
        for c in criteria:
    ...
    def _test_single_criterion(self, criterion: AcceptanceCriterion, tree: ast.AST, func_names: set[str]) -> FuzzTestResult:
        ...
        if criterion.category == 'error':
            for node in ast.walk(tree):
                if isinstance(node, (ast.Try, ast.ExceptHandler)):
                    return FuzzTestResult(criterion_id=criterion.criterion_id, description=desc, category=criterion.category, passed=True)
            return FuzzTestResult(criterion_id=criterion.criterion_id, description=desc, category=criterion.category, passed=False, error='No try/except error handling found')
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.If):
                    ...
                    for kw in ('None', 'len', '==', 'is None', '<', '>', 'not'):
                        if kw in test_str:
                            return FuzzTestResult(criterion_id=criterion.criterion_id, description=desc, category=criterion.category, passed=True)
            return FuzzTestResult(criterion_id=criterion.criterion_id, description=desc, category=criterion.category, passed=False, error='No boundary condition check found')
        return FuzzTestResult(criterion_id=criterion.criterion_id, description=desc, category=criterion.category, passed=True)

    def _extract_keywords(self, description: str) -> list[str]:
        keywords: list[str] = []
        for pair in (('登录', 'login'), ('注册', 'register'), ('搜索', 'search'), ('上传', 'upload'), ('成功', 'success'), ('失败', 'fail'), ('锁定', 'lock'), ('拒绝', 'reject'), ('空', 'empty')):
            if pair[0] in description or pair[1] in description.lower():
                keywords.append(pair[1])
        return keywords or [description[:20].lower()]

    def _extract_negative_gene(self, failure: FuzzTestResult, code: str, language: str) -> str | None:
