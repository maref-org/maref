...
class NegativeGeneBank:
    ...
    def get_gene(self, gene_id: str) -> NegativeGene | None:
        row = self._conn.execute(
            "SELECT * FROM negative_genes WHERE gene_id=?", (gene_id,)
        ).fetchone()
        if row is None:
            return None
    ...
    def query_all(self, limit: int = 1000, offset: int = 0) -> list[NegativeGene]:
        rows = self._conn.execute(
            "SELECT * FROM negative_genes ORDER BY gene_id LIMIT ? OFFSET ?", (limit, offset)
        ).fetchall()
    ...
    def count_by_cwe(self) -> dict[str, int]:
        rows = self._conn.execute(
            "SELECT cwe_id, COUNT(*) FROM negative_genes GROUP BY cwe_id ORDER BY COUNT(*) DESC"
        ).fetchall()
        return dict(rows)

    def count_by_risk(self) -> dict[str, int]:
        rows = self._conn.execute(
            "SELECT risk_level, COUNT(*) FROM negative_genes GROUP BY risk_level"
        ).fetchall()
        return dict(rows)

    def count_by_source(self) -> dict[str, int]:
        rows = self._conn.execute(
            "SELECT source, COUNT(*) FROM negative_genes GROUP BY source"
        ).fetchall()
        return dict(rows)
    ...
    def register_variant(self, gene_id: str, variant: GeneVariant) -> None:
        ...
        with self._conn:
            ...
            if row is not None:
                ...
                gene.update_hmac(self._hmac_key)
                self._conn.execute(
                    "UPDATE negative_genes SET hmac_signature=?, updated_at=? WHERE gene_id=?",
                    (gene.hmac_signature, time.time(), gene_id),
                )
    ...
    def increment_occurrence(self, gene_id: str) -> None:
        ...
        gene.update_hmac(self._hmac_key)
        self._conn.execute(
            "UPDATE negative_genes SET occurrences=?, hmac_signature=?, updated_at=? WHERE gene_id=?",
            (gene.occurrences, gene.hmac_signature, time.time(), gene_id),
        )

    def record_source_import(
        self, source_name: str, source_url: str = "", gene_count: int = 0
    ) -> str:
        """Record a CWE source import in the gene_sources table. Returns source_id."""
        src_id = f"SRC-{uuid.uuid4().hex[:12].upper()}"
        self._conn.execute(
            "INSERT OR REPLACE INTO gene_sources\n               (source_id, source_name, source_url, import_date, gene_count, schema_version)\n               VALUES (?, ?, ?, ?, ?, ?)",
            (src_id, source_name, source_url, time.time(), gene_count, self._SCHEMA_VERSION),
        )
        self._conn.commit()
        return src_id
    ...
    def get_import_history(self) -> list[dict[str, Any]]:
        ...
        rows = self._conn.execute(
            "SELECT source_id, source_name, source_url, import_date, gene_count, schema_version FROM gene_sources ORDER BY import_date DESC"
        ).fetchall()
        return [
            {
                "source_id": r[0],
    ...
    def get_gene_lifecycle(self, gene_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "cwe_id": gene.cwe_id,
            "risk_level": gene.risk_level.value
            if hasattr(gene.risk_level, "value")
            else gene.risk_level,
            ...
            "hmac_valid": gene.verify_hmac(self._get_hmac_key())
            if hasattr(gene, "verify_hmac")
            else "unknown",
        }
