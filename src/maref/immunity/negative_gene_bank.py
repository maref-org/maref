...
class NegativeGene:
    ...
    def update_hmac(self, key: bytes) -> None:
        payload = (
            f"{self.gene_id}|{self.cwe_id}|{self.risk_level}|{self.severity}|"
            f"{int(self.blocked)}|{self.title}|{self.description}|{self.source}|"
            f"{self.first_seen}|{self.occurrences}|{self.retention_days}"
        )
...
class NegativeGeneBank:
    ...
    def query_all(self, limit: int = 1000, offset: int = 0) -> list[NegativeGene]:
        rows = self._conn.execute(
            "SELECT * FROM negative_genes ORDER BY gene_id LIMIT ? OFFSET ?",
            (limit, offset),
        ).fetchall()
    ...
    def get_import_history(self) -> list[dict[str, Any]]:
        ...
        rows = self._conn.execute(
            "SELECT source_id, source_name, source_url, import_date, gene_count, schema_version "
            "FROM gene_sources ORDER BY import_date DESC"
        ).fetchall()
        return [
            {
                "source_id": r[0],
