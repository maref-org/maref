...
_WRONG_COMMENT_MAP: list[tuple[str, list[str]]] = [('eval', ['use eval', 'using eval is safe', 'eval is secure']), ('pickle', ['safe to unpickle', 'secure pickle', 'pickle is safe']), ('md5', ['md5 is secure', 'md5 encryption', 'secure md5']), ('sha1', ['sha1 is secure', 'SHA1 secure', 'sha1 encryption']), ('exec(', ['use exec', 'using exec', 'exec is safe'])]
...
class RedContaminationProbe:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                if 'pickle' in ' '.join(names):
                    findings.append(ContaminationFinding(type='deprecated_pickle', severity='POLLUTION', line=node.lineno or 0, message='pickle import detected — insecure deserialization can execute arbitrary code', suggestion='Replace pickle with JSON (json.loads/json.dumps) or MessagePack', code_snippet=ast.unparse(node)[:100]))
            if isinstance(node, ast.Call):
                name = ast.unparse(node.func)
                if 'pickle.' in name:
                    findings.append(ContaminationFinding(type='deprecated_pickle', severity='POLLUTION', line=node.lineno or 0, message=f"pickle operation '{name}' — insecure deserialization risk", suggestion='Replace pickle with JSON or another safe serialization format', code_snippet=ast.unparse(node)[:100]))
        return findings
    ...
    def _write_to_audit(self) -> None:
        ...
        for (i, f) in enumerate(self._findings):
            record = UnifiedAuditRecord(record_id=f'contam_{i:06d}_{int(ts * 1000)}', timestamp=ts, layer='execution', round=0, event_type=f'contamination_{f.type}', source_module='red_contamination_probe', target_module='code_analysis', decision=f.severity, justification=f.message, outcome=None, context_refs=[])
