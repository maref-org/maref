...
class CrossGenerationImpactSimulator:
    ...
    def simulate_contamination(self, code: str) -> ContaminationReport:
        ...
        if not findings:
            report = ContaminationReport(contamination_index=0.0, blocked=False, findings=[], details={'reason': 'No contamination patterns detected'})
        ...
        details: dict[str, Any] = {'type_counts': dict(type_counts), 'weighted_score': weighted_score, 'synergy_applied': len(type_counts) >= 3, 'total_findings': len(findings)}
        report = ContaminationReport(contamination_index=contamination_index, blocked=blocked, findings=findings, details=details)
        self._write_audit(report)
        return report
    ...
    def simulate_training_impact(self, code: str) -> dict[str, Any]:
        ...
        for f in report.findings:
            teachable.append({'pattern': f.type, 'teachability': self._estimate_teachability(f), 'message': f.message, 'suggestion': f.suggestion})
        risk_level = 'critical' if report.blocked else 'moderate'
        impact = f"Code has {len(report.findings)} contamination patterns. {('MERGE BLOCKED' if report.blocked else 'Acceptable risk')}. Training on this code would teach AI to use {', '.join({f.type for f in report.findings})}."
        return {'teachable_patterns': teachable, 'risk_level': risk_level, 'impact': impact, 'contamination_index': report.contamination_index}
