...
class IntentHash:
    ...
    criteria_count: int
    generated_at: float
TEST_TEMPLATES: dict[str, str] = {'happy_path': 'def test_{safe_desc}():\n    result = implement({args})\n    assert result is not None\n    assert result["status"] == "success"\n', 'error': 'def test_{safe_desc}():\n    with pytest.raises({exception}):\n        implement({args})\n', 'boundary': 'def test_{safe_desc}():\n    result = implement({args})\n    assert result is not None\n    assert result["status"] == "success"\n'}

def _make_test_template(description: str, category: str, exception: str='ValueError', args: str='') -> str:
