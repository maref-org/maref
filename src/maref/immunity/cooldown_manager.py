...
class CooldownManager:
    def __init__(
        self,
        simulator: CrossGenerationImpactSimulator | None = None,
    ...
    def submit_code(self, agent_id: str, code: str, metadata: dict[str, Any] | None = None) -> str:
        ...
        entry = CooldownEntry(
            cooldown_id=cooldown_id, agent_id=agent_id, code=code, metadata=metadata or {}
        )
    ...
    def get_status(self, cooldown_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "contamination_index": entry.contamination_index,
            "blocked": entry.blocked,
            "merged": entry.merged,
            "force_merged": entry.force_merged,
        }
    ...
    def evaluate(self, cooldown_id: str) -> dict[str, Any]:
        ...
        if report.blocked:
            ...
            self._write_audit(
                "blocked",
                cooldown_id,
                f"Contamination index {report.contamination_index} >= 0.7, merge blocked",
            )
        else:
    ...
    def auto_merge(self, cooldown_id: str) -> dict[str, Any]:
        ...
        if elapsed < self._cooldown_seconds:
            return {
                "success": False,
                "reason": f"cooldown not finished ({elapsed:.0f}s < {self._cooldown_seconds}s)",
            }
        if entry.blocked:
    ...
    @security_critical
    def force_merge(
        self, cooldown_id: str, actor_id: str = "system", reason: str = "manual_override"
    ) -> dict[str, Any]:
        entry = self._entries.get(cooldown_id)
        if entry is None:
            return {"success": False, "reason": "cooldown_id not found"}
    ...
    def auto_archive_expired(self, max_age_days: int = 7) -> list[str]:
        ...
        for entry in list(self._entries.values()):
            ...
            if age_days >= max_age_days:
                ...
                self._write_audit(
                    "cooldown_archived", entry.cooldown_id, f"auto_archive after {max_age_days}d"
                )
    ...
    def _write_audit(self, event_type: str, cooldown_id: str, justification: str) -> None:
        ...
        self._audit_store.append(
            UnifiedAuditRecord(
                record_id=f"cooldown_{event_type}_{int(time.time() * 1000)}",
                timestamp=time.time(),
                layer="execution",
                round=0,
                event_type=f"cooldown_{event_type}",
                source_module="CooldownManager",
                target_module=entry.agent_id if entry else "unknown",
