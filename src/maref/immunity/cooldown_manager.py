...
class CooldownManager:
    ...
    def force_merge(self, cooldown_id: str, reason: str = "manual_override") -> dict[str, Any]:
        ...
        if entry.blocked and entry.contamination_index >= 0.7:
            notification = (
                f"FORCE MERGE of contaminated code (index={entry.contamination_index:.2f}) "
                f"by agent {entry.agent_id}. Reason: {reason}"
            )
        else:
            notification = (
                f"Force merge of clean or unevaluated code by agent {entry.agent_id}. "
                f"Reason: {reason}"
            )
