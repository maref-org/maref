...
def seed_from_cwe_json(
    ...
    if source_name not in SUPPORTED_SOURCES:
        raise CWEImportError(
            f"Unknown source '{source_name}'. Supported: {sorted(SUPPORTED_SOURCES)}"
        )
    ...
    if isinstance(data, list):
        ...
        raise CWEImportError(
            "Unknown JSON structure. Expected a list of gene entries, or a dict with 'genes'/'cwe_entries' key."
        )
    imported = 0
    updated = 0
    skipped = 0
    ...
    for entry in entries:
        ...
        patterns = [
            GenePattern(
                ...
                variant_group=p.get("group", "primary"),
                match_score=p.get("score", 1.0),
            )
            ...
            "patterns": [
                {
                    ...
                    "group": p.variant_group,
                    "score": p.match_score,
                }
                for p in g.patterns
            ],
            "variants": [{"language": v.language, "code": v.variant_code} for v in g.variants],
        }
