...
class AutoGeneExtractionPipeline:
    ...
    def extract_from_heal(self, snapshot: str, fix_code: str, reason: str='') -> str | None:
        gene = self._build_gene(title='Self-healed code pattern fixed', description=f"Auto-heal corrected pattern. Reason: {reason or 'unknown'}", cwe_id='CWE-1104', risk_level='MEDIUM', severity=5, blocked=False, source='auto_heal', pattern_value=self._pattern_from_diff(snapshot, fix_code))
        if gene is None:
            return None
    ...
    def extract_from_rollback(self, code_snapshot: str, reason: str='') -> str | None:
        gene = self._build_gene(title='Code pattern caused rollback', description=f"Auto-extracted from rollback. Reason: {reason or 'unknown'}", cwe_id='CWE-1104', risk_level='HIGH', severity=7, blocked=True, source='self_executor_rollback', pattern_value=self._pattern_from_code(code_snapshot))
        if gene is None:
            return None
        gene_id = self._gene_bank.store_gene(gene)
    ...
    def extract_from_block(self, blocked_code: str, reason: str='') -> str | None:
        gene = self._build_gene(title='Code pattern blocked by SafetyGate', description=f"Auto-extracted from gate block. Reason: {reason or 'unknown'}", cwe_id='CWE-1104', risk_level='HIGH', severity=8, blocked=True, source='safety_gate_v2', pattern_value=self._pattern_from_code(blocked_code))
        if gene is None:
            return None
        gene_id = self._gene_bank.store_gene(gene)
    ...
    def _build_gene(self, title: str, description: str, cwe_id: str, risk_level: str, severity: int, blocked: bool, source: str, pattern_value: str) -> Any | None:
    ...
    def _record_extraction(self, source: str, gene_id: str, reason: str) -> None:
        ...
        self._recent_extractions.append({'gene_id': gene_id, 'source': source, 'reason': reason, 'timestamp': time.time()})

    def _sync_to_experience(self, gene: Any, tag_hint: str) -> None:
        from maref.recursive.experience_pool import ExperienceEntry
        entry = ExperienceEntry(entry_id=f'ext_{uuid.uuid4().hex[:8]}', timestamp=time.time(), context=f'Auto-extracted negative gene: {gene.title} ({gene.source})', decision='EXTRACT', outcome='stored', lesson_learned=gene.description[:200], tags=[f'auto_gene:{gene.source}', f'cwe:{gene.cwe_id}'])
        self._experience_pool.store(entry)

    def _sync_to_experience_from_record(self, extraction: dict[str, Any]) -> None:
        from maref.recursive.experience_pool import ExperienceEntry
        entry = ExperienceEntry(entry_id=f'sync_{uuid.uuid4().hex[:8]}', timestamp=time.time(), context=f"Synced extraction: {extraction['gene_id']} ({extraction['source']})", decision='SYNC', outcome='synced', lesson_learned=f"Auto-synced from {extraction['source']}: {extraction['reason']}", tags=[f"auto_gene:{extraction['source']}", 'sync'])
        self._experience_pool.store(entry)