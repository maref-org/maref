...
class CapabilityViolation(SandboxError):
    """能力违规"""
    pass
...
class SandboxCapabilities:
    ...
    def validate_access(self, capability: str, details: Optional[dict] = None) -> bool:
        """验证是否允许某项能力"""
        capability_map = {
...
class WasmSandboxExecutor:
    ...
    def _execute_with_wasmtime(
        ...
        capabilities: Optional[SandboxCapabilities],
    ) -> ExecutionResult:
        """使用 wasmtime 执行 WASM"""
        # 写入临时文件
...
def create_wasm_sandbox(limits: Optional[ResourceLimits] = None) -> WasmSandboxExecutor:
    """创建 WASM 沙箱执行器"""
