...
_IMMUNE_PATTERNS: list[tuple[str, str, int, str]] = [
    ("hardcoded_secret", "Hardcoded password or API key", 5,
     r"""(?x)
        (password|secret|api_key|token|credential)\s*=\s*['\"][^'\"]{4,}['\"]
    """),
    ("insecure_hash", "Insecure hash algorithm (MD5/SHA1)", 4,
     r"\b(hashlib\.md5|hashlib\.sha1|import\s+md5)\b"),
    ("eval_exec", "Use of eval/exec on untrusted input", 5,
     r"\b(eval|exec)\s*\("),
    ("sql_injection", "Possible SQL injection (string formatting in query)", 5,
     r"""(?x)
        (execute|executemany)\s*\(\s*['\"](?:.|\n)*?\{.*?\}
    """),
    ("pickle_load", "Unsafe pickle deserialization", 4,
     r"\b(pickle\.load|pickle\.loads|cPickle\.load|cPickle\.loads)\s*\("),
    ("assert", "assert statement used in production path", 3,
     r"^\s*assert\s+"),
    ("print_to_stdout", "print statement in production code", 2,
     r"^\s*print\s*\("),
    ("bare_except", "Bare except clause", 3,
     r"^\s*except\s*:"),
    ("shell_injection", "os.system/subprocess with shell=True", 4,
     r"""(?x)
        (os\.system|subprocess\.call|subprocess\.Popen|subprocess\.run)\s*\(
        (?:.|\n)*?shell\s*=\s*True
    """),
    ("mutable_default_arg", "Mutable default argument", 2,
     r"""(?x)
    ...
    async def call(self, input: ImmuneScanInput, ctx: ToolContext) -> ToolResult[ImmuneScanOutput]:
        ...
        if self._immune_checker is not None:
            try:
                ...
                return ToolResult(
                    data=ImmuneScanOutput(hits=[], blocked=False, scan_count=0),
                    metadata={"fallback": "Checker failed, using built-in patterns", "error": str(e)},
                )
