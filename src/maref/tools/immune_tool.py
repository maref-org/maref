...
class ImmuneScanTool(Tool[ImmuneScanInput, ImmuneScanOutput]):
    ...
    async def validate(self, input: ImmuneScanInput) -> ValidationResult:
        if not input.code.strip() and (not input.file_path):
            return ValidationResult(is_valid=False, message='Either code or file_path must be provided')
    ...
    async def call(self, input: ImmuneScanInput, ctx: ToolContext) -> ToolResult[ImmuneScanOutput]:
        ...
        if input.file_path and (not code):
            try:
                ...
                return ToolResult(data=ImmuneScanOutput(hits=[], blocked=False, scan_count=0), metadata={'error': f'Failed to read file: {e}'})
    ...
    async def _scan_with_builtin_patterns(self, code: str) -> ToolResult[ImmuneScanOutput]:
        ...
        for (gene_id, title, severity, pattern) in _IMMUNE_PATTERNS:
            for match in re.finditer(pattern, code, re.MULTILINE):
                ...
                risk = 'critical' if severity >= 5 else 'high' if severity >= 4 else 'medium' if severity >= 3 else 'low'
                hits.append(ImmuneHit(gene_id=gene_id, gene_title=title, risk_level=risk, severity=severity, blocked=severity >= 5, match_type='regex', match_location=(line_no, col), match_snippet=context_line, fix_suggestion=self._get_fix_suggestion(gene_id)))
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    ...
                    if isinstance(func, ast.Attribute) and func.attr == 'system' and isinstance(func.value, ast.Name) and (func.value.id == 'os'):
                        hits.append(ImmuneHit(gene_id='os_system', gene_title='os.system call', risk_level='high', severity=4, blocked=False, match_type='ast', match_location=(node.lineno, node.col_offset), match_snippet=f'os.system() at line {node.lineno}', fix_suggestion='Use subprocess.run() instead'))
                    if isinstance(func, ast.Attribute) and func.attr == 'setdefaultencoding' and isinstance(func.value, ast.Name) and (func.value.id == 'sys'):
                        hits.append(ImmuneHit(gene_id='setdefaultencoding', gene_title='sys.setdefaultencoding() called', risk_level='high', severity=4, blocked=True, match_type='ast', match_location=(node.lineno, node.col_offset), match_snippet='sys.setdefaultencoding() at line {node.lineno}', fix_suggestion='Remove this call - Python 3 uses UTF-8 by default'))
        except SyntaxError:
            pass
        return hits

    def _get_fix_suggestion(self, gene_id: str) -> str | None:
        suggestions = {'hardcoded_secret': 'Use environment variables or Keychain for secrets', 'insecure_hash': 'Use hashlib.sha256 or stronger', 'eval_exec': 'Use ast.literal_eval() or a safer alternative', 'sql_injection': 'Use parameterized queries with ? placeholders', 'pickle_load': 'Use json or a safer serialization format', 'assert': 'Replace assert with proper if/raise for defensive checks', 'print_to_stdout': 'Use logging module instead of print()', 'bare_except': 'Always specify exception type(s) to catch', 'shell_injection': 'Avoid shell=True; pass arguments as a list', 'mutable_default_arg': 'Use None default with None-guard inside function', 'os_system': 'Use subprocess.run() instead'}
