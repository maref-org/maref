...
class ObserveTool(Tool[str]):
    @property
    def name(self) -> str:
        return "maref_observe"

    @property
    def description(self) -> str:
        return "Take a system snapshot — module graph, test stats, git state, probe readings"
...
class DiagnoseTool(Tool[str]):
    @property
    def name(self) -> str:
        return "maref_diagnose"

    @property
    def description(self) -> str:
        return "Run diagnostics on a system snapshot — risk matrix, CB status, recommendations"
...
class ArchitectTool(Tool[str]):
    @property
    def name(self) -> str:
        return "maref_architect"

    @property
    def description(self) -> str:
        return "Generate architecture proposals — refactoring, test addition, import cleanup"
...
class CodegenTool(Tool[str]):
    @property
    def name(self) -> str:
        return "maref_codegen"

    @property
    def description(self) -> str:
        return "Generate code from an architecture proposal using LLM"
    ...
    async def execute(self, input: dict[str, Any], context: ToolUseContext) -> ToolResult[str]:
        ...
        proposal = ArchitectureProposal(
            ...
            rationale=input.get("rationale", "generated by CodegenTool"),
            risk_assessment="low",
            confidence=0.8,
        )
...
class DeployTool(Tool[str]):
    ...
    @property
    def description(self) -> str:
        return "Execute a proposal through the full pipeline — code gen, AST validate, safety gate, deploy, verify"
    ...
    async def execute(self, input: dict[str, Any], context: ToolUseContext) -> ToolResult[str]:
        ...
        proposal = ArchitectureProposal(
            ...
            timestamp=__import__("time").time(),
            current_arch=proposal_data.get("current_arch", "current"),
            proposed_arch=proposal_data.get("proposed_arch", "proposed"),
            rationale=proposal_data.get("rationale", "deployed by DeployTool"),
            risk_assessment=proposal_data.get("risk_assessment", "low"),
...
class VerifyTool(Tool[str]):
    ...
    @property
    def description(self) -> str:
        return "Run test suite and quality checks"
